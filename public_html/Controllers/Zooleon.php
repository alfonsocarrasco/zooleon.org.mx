<?php

include_once('Models/TDataGral.php');

class Zooleon extends Controllers {

    use TDataGral;

    public function __construct() {
        parent::__construct();
        mantenimiento();
    }

    public function zooleon() {

        $infoHistoria = $this->getDataHistory();

        $data['page_id']       = 1;
        if(!empty($infoHistoria)) {
            $data['page_content']  = $infoHistoria['descripcion_h'];
        }
        $data['page_tag']      = 'historia';
        $data['page_title']    = 'Historia | '.NOMBRE_EMPRESA;
        $data['page_name']     = 'Historia';
        $data['page_keywords'] = 'Zoológico, Zoológico León, Especies, Atracciones, Paquetes, Paquete Integral, Paquete Zoológico, Paquete Safari, Perro Parque, Zoona Lemur, Safari, Zoona Reptil, Zoona Mito, Cabaña de Tío Bufalo, Aves, Mamiferos, Reptiles, Tren, Ranchito, León Guanajuato, León, Guanajuato';
        $data['info_historia'] = $infoHistoria;
        $data['infogral']      = $this->getDataInfoGral();
        $this->views->getView($this, 'zooleon', $data);
    }

}