...
class Pagenacimientos extends Controllers {
    ...
    public function pagenacimientos() {
        ...
        $data['page_functions_js'] = 'functions_pagenacimientos.js';
        $this->views->getView($this, 'pagenacimientos', $data);
            ...
            if (empty($_POST['txtTitulo']) || empty($_POST['txtNameEspecie']) || empty($_POST['txtNameScien']) || empty($_POST['listStatus'])) {
                ...
                if ($intIdPage == 0) {
                    ...
                    if ($getPermisos['permisosMod']['w']) {
                        $request = $this->model->insertDataPage($intIdPage, $strTitulo, $strNameEspecie, $strnameScientific, $imgPortada, $imgPortada_parallax, $intStatus);
                        $option = 1;
                    }
    ...
    public function getDataPage($id = null) {
        ...
        if($getPermisos['permisosMod']['r']) {
            ...
            if(empty($arrData)) {
                ...
                if ($arrData['parallax'] == 'portada_categoria.jpg') {
                    $arrData['portada_url_parallax'] = media().'images/'.$arrData['parallax'];
                } else {
                    $arrData['portada_url_parallax'] = media().'images/uploads/'.$arrData['parallax'];
                }
