...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (document.querySelector("#formPageReglamento")) {
        ...
        formPageReglamento.onsubmit = function(e) {
            ...
            let ajaxUrl = base_url + 'blog/setDataPageNoticias';
...
function fntViewPageNoticias() {
    let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    let ajaxUrl = base_url + 'blog/getDataPageNoticias/';
    ...
    request.onreadystatechange = function() {
        if (request.readyState == 4 && request.status == 200) {
            ...
            if (objData.status) {
                ...
                const { idpagenoticia, namescie_pagenoticia, namespecie_pagenoticia, portada_url, portada_url_parallax, statuspagenoticia, titulo_pagenoticia } = objData.data;

                let status = statuspagenoticia == 1 ? '<span class="badge rounded-pill bg-success">Activo</span>' : '<span class="badge rounded-pill bg-danger">Inactivo</span>';

                if (document.querySelector('.edit')) {
                    document.querySelector('.edit').innerHTML = '<button type="button" class="btn btn-dark btn-ecom" onclick="fntEditPageNoticias('+ idpagenoticia +');"><i class="fa-solid fa-pen-to-square"></i> Editar</button>';
                }
                
                document.querySelector('.title').innerHTML            = titulo_pagenoticia;
                document.querySelector('.nameespecie').innerHTML      = namespecie_pagenoticia;
                document.querySelector('.namescientific').innerHTML   = namescie_pagenoticia;
...
function fntEditPageNoticias(id) {
    ...
    let request = (window.XMLHttpRequest) ? new XMLHttpRequest() : new ActiveXObject('Microsoft.XMLHTTP');
    let ajaxUrl = base_url + 'blog/getDataPageNoticias/' + id;
    request.open('GET', ajaxUrl, true);
    request.send();
    request.onreadystatechange = function() {
        ...
        if (request.readyState == 4 && request.status == 200) {
            ...
            if (objData.status) {
                ...
                const { idpagenoticia, namescie_pagenoticia, namespecie_pagenoticia, parallax_pagenoticia, portada_pagenoticia, portada_url, portada_url_parallax, statuspagenoticia, titulo_pagenoticia } = objData.data;
                
                document.querySelector('#idpageregla').value          = idpagenoticia;
                document.querySelector('#txtTitulo').value            = titulo_pagenoticia;
                document.querySelector('#txtNameEspecie').value       = namespecie_pagenoticia;
                document.querySelector('#txtNameScien').value         = namescie_pagenoticia;
                document.querySelector('#foto_actual').value          = portada_pagenoticia;
                document.querySelector('#foto_remove').value          = 0;
                document.querySelector('#foto_actual_parallax').value = parallax_pagenoticia;
                ...
                if (portada_pagenoticia == 'portada_categoria.jpg' || portada_pagenoticia == '') {
                    document.querySelector('.delPhoto').classList.add("notBlock");
                } else {
                    document.querySelector('.delPhoto').classList.remove("notBlock");
                }
                
                if (parallax_pagenoticia == 'portada_categoria.jpg' || parallax_pagenoticia == '') {
                    document.querySelector('.delPhoto2').classList.add("notBlock");
                } else {
                    document.querySelector('.delPhoto2').classList.remove("notBlock");
                }

                if (statuspagenoticia == 1) {
...
function openModalPage() {
    ...
    document.querySelector('#titleModal').innerHTML = "Agregar Información Página Noticias";
